...
function getBaseRate(area: number): number {
  const sortedRates = [...areaRates].sort((a, b) => b.area - a.area);
...
export default function Window3TrackPage() {
  ...
  const [glassType, setGlassType] = useState<"clear" | "reflective">("clear");
  const [chaukhat, setChaukhat] = useState(false);
  ...
  const calculations = useMemo(() => {
    ...
    const baseRate = getBaseRate(area);
    const thicknessRates = baseRates[selectedThickness];
  ...
  }, [height, width, heightSoot, widthSoot, glassType, chaukhat, decorFilm, ssNet, brownCoated, selectedThickness, baseRates]);
  ...
  const clearRateHindalco = calculations.baseRate + baseRates["1.2 mm Hindalco"].clear;
  const reflectiveRate1_2 = calculations.baseRate + baseRates["1.2 mm"].reflective;
  const reflectiveRate1_6 = calculations.baseRate + baseRates["1.6 mm"].reflective;
  ...
  return (
    <div className="min-h-screen text-white relative overflow-hidden">
      ...
      <main className="relative mx-auto max-w-7xl px-6 py-12">
        ...
        <div className="glass-panel mb-8 space-y-3 px-6 py-8 text-center">
          <h1 className="font-serif text-4xl md:text-5xl font-bold">3 Track Aluminium Sliding Window</h1>
          <p className="text-white/70 text-lg">With Half SS Net — curated for lobby entries and premium balconies.</p>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setSelectedThickness(e.target.value as keyof typeof baseRates)}
                className="w-full rounded border-2 border-white bg-black px-3 py-2 text-white"
              >
            ...
            <div>
              <label className="mb-2 block text-sm font-medium">Glass Type</label>
              <select
                value={glassType}
                onChange={(e) => setGlassType(e.target.value as "clear" | "reflective")}
                className="w-full rounded border-2 border-white bg-black px-3 py-2 text-white"
              >
                <option value="clear">4 MM Clear & Texture Glass</option>
                <option value="reflective">4 MM Reflective Glass</option>
              </select>
            </div>
          </div>
          ...
          <table className="w-full border-2 border-white">
            ...
            <tbody>
              <tr className="border-b border-white">
                <td className="border-r-2 border-white p-2">4 MM Clear & Texture Glass</td>
                <td className="border-r-2 border-white p-2 text-center">
                  @{clearRate1_2}
                </td>
                <td className="border-r-2 border-white p-2 text-center">
                  {selectedThickness === "1.2 mm" && glassType === "clear" ? `₹${Math.round(calculations.glassCost)}` : ""}
                </td>
                <td className="border-r-2 border-white p-2 text-center">
                  @{clearRate1_6}
                </td>
                <td className="border-r-2 border-white p-2 text-center">
                  {selectedThickness === "1.6 mm" && glassType === "clear" ? `₹${Math.round(calculations.glassCost)}` : ""}
                </td>
                <td className="border-r-2 border-white p-2 text-center">
                  @{clearRateHindalco}
                </td>
                <td className="p-2 text-center">
                  {selectedThickness === "1.2 mm Hindalco" && glassType === "clear" ? `₹${Math.round(calculations.glassCost)}` : ""}
                </td>
                <td className="p-2"></td>
              </tr>
              <tr className="border-b border-white">
                ...
                <td className="border-r-2 border-white p-2"></td>
                <td className="border-r-2 border-white p-2"></td>
                <td className="border-r-2 border-white p-2 text-center">@85</td>
                <td className="border-r-2 border-white p-2 text-center">
                  {chaukhat && selectedThickness === "1.6 mm" ? `₹${Math.round(calculations.chaukhatCost)}` : ""}
                </td>
                <td className="border-r-2 border-white p-2 text-center">@85</td>
                <td className="p-2 text-center">
                  {chaukhat && selectedThickness === "1.2 mm Hindalco" ? `₹${Math.round(calculations.chaukhatCost)}` : ""}
                </td>
                <td className="p-2">
                  <input
                    type="checkbox"
              ...
              <tr className="border-b border-white">
                <td className="border-r-2 border-white p-2">4 MM Reflective Glass</td>
                <td className="border-r-2 border-white p-2 text-center">
                  @{reflectiveRate1_2}
                </td>
                <td className="border-r-2 border-white p-2 text-center">
                  {selectedThickness === "1.2 mm" && glassType === "reflective" ? `₹${Math.round(calculations.glassCost)}` : ""}
                </td>
                <td className="border-r-2 border-white p-2 text-center">
                  @{reflectiveRate1_6}
                </td>
                <td className="border-r-2 border-white p-2 text-center">
                  {selectedThickness === "1.6 mm" && glassType === "reflective" ? `₹${Math.round(calculations.glassCost)}` : ""}
                </td>
                <td className="border-r-2 border-white p-2 text-center">
                  @{reflectiveRateHindalco}
                </td>
                <td className="p-2 text-center">
                  {selectedThickness === "1.2 mm Hindalco" && glassType === "reflective" ? `₹${Math.round(calculations.glassCost)}` : ""}
                </td>
                <td className="p-2"></td>
              </tr>
        ...
        <div className="mb-8">
          ...
          <table className="w-full">
            <tbody>
              ...
              <tr className="border-b border-white">
                <td className="border-r-2 border-white p-2">SS Net</td>
                <td className="border-r-2 border-white p-2 text-center">@50</td>
                <td className="border-r-2 border-white p-2 text-center">
                  {ssNet ? `₹${Math.round(calculations.ssNetCost)}` : ""}
                </td>
                <td className="border-r-2 border-white p-2 text-center">@50</td>
                <td className="border-r-2 border-white p-2 text-center">
                  {ssNet ? `₹${Math.round(calculations.ssNetCost)}` : ""}
              ...
              <tr>
                ...
                <td className="border-r-2 border-white p-2 text-center">@40</td>
                <td className="border-r-2 border-white p-2 text-center">
